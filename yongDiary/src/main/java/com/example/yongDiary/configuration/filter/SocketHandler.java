...
// -> service나 dao 같은게 아니라서 bean으로 등록해줘야함
@Component
public class SocketHandler extends TextWebSocketHandler {
	...
	// 웹소켓 세션 ID와 Member 담아둘 맵
	// 맵 말고 DB 형식으로 담아두기 위해서는 변하지 않는 값(유일성) 담아둬야함  
	...
	// 2. handleTextMessage 메소드 -> 메세지를 서버에서 수신하면 실행
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		case "userSave":
			...
			System.out.printf("arrayJsonUesr : %s", arrayJsonUser);
	...
	// json으로 parsing 하지 않으면 string을 잘라서 사용해야하기 때문에 해줘야함
